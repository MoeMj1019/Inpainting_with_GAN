...
def inspect_data(HR_path, LR_path, result_path=None, type_end='.png',
    ...
    fig_scale = 5
    if result_path is not None:
        fig , axes = plt.subplots(sample_images,4,figsize=(fig_scale*3,fig_scale*sample_images))

        for idx, (ax1,ax2,ax3,ax4) in enumerate(axes):
            ...
            ax4.imshow(np.abs(img_LR - img_result))
        ...
        for idx, (ax1, ax2) in enumerate(axes):
            ...
            if idx == 0:
                ax1.set_title('GT image')
                ax2.set_title('incomplete image')
